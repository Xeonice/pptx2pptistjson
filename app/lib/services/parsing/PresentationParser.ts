...
import { Presentation, PresentationMetadata, SlideSize } from '../../models/domain/Presentation';
...
export class PresentationParser implements IPresentationParser {
  ...
  async parse(file: ArrayBuffer | Blob, options?: ParseOptions): Promise<ParseResult> {
    ...
    try {
      ...
      for (const slideFile of slideFiles) {
        try {
          ...
          warnings.push({
            level: 'error',
            message: `Failed to parse slide ${slideFile}: ${(error as Error).message}`,
            slideNumber
          });
        }
      }
      ...
      const stats = {
        ...
        totalElements: presentation.getSlides().reduce(
          (sum, slide) => sum + slide.getElements().length,
          0
        ),
        elementCounts: this.calculateElementCounts(presentation),
        parseTimeMs: Date.now() - startTime,
        fileSizeBytes: file instanceof ArrayBuffer ? file.byteLength : file.size
      };
      ...
      throw new Error(`Failed to parse presentation: ${(error as Error).message}`);
    }
  }

  private async parseContentTypes(zip: JSZip): Promise<Map<string, string>> {
    const contentTypesXml = await this.fileService.extractFile(zip, '[Content_Types].xml');
  ...
  private async parsePresentationProperties(zip: JSZip, presentation: Presentation): Promise<void> {
    try {
      // Parse presentation.xml for slide size
      if (this.fileService.fileExists(zip, 'ppt/presentation.xml')) {
        const presentationXml = await this.fileService.extractFile(zip, 'ppt/presentation.xml');
        ...
        if (sldSzNode) {
          ...
          if (cx && cy) {
            ...
            const slideSize: SlideSize = {
              width: Math.round(parseInt(cx) / 12700),
              height: Math.round(parseInt(cy) / 12700)
            };
      ...
      if (this.fileService.fileExists(zip, 'docProps/core.xml')) {
        const coreXml = await this.fileService.extractFile(zip, 'docProps/core.xml');
  ...
  private async parseRelationships(zip: JSZip, slidePath: string): Promise<Map<string, any>> {
    ...
    try {
      ...
      if (this.fileService.fileExists(zip, relsPath)) {
        ...
        const relationshipNodes = this.xmlParser.findNodes(relsNode, 'Relationship');
  ...
  private calculateElementCounts(presentation: Presentation): Record<string, number> {
