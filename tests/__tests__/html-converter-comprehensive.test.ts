...
} from "../../app/lib/services/utils/HtmlConverter";
import {
  TextContent,
  TextRunStyle,
} from "../../app/lib/models/domain/elements/TextElement";
...
describe("HtmlConverter - Comprehensive Test Suite", () => {
  describe("Single Paragraph Conversion", () => {
    it("should convert simple text content to HTML", () => {
      const content: TextContent[] = [{ text: "Hello World", style: {} }];

      const html = HtmlConverter.convertSingleParagraphToHtml(content);
      // 现在 HtmlConverter 始终包含默认字体大小 (23.99px)
      expect(html).toBe(
        '<div style=""><p style=""><span style="font-size:23.99px">Hello World</span></p></div>'
      );
    });
    ...
    it("should handle multiple text runs in a paragraph", () => {
      ...
      expect(html).toContain(
        '<span style="font-size:23.99px;font-weight:bold">Bold </span>'
      );
      expect(html).toContain(
        '<span style="font-size:23.99px;font-style:italic">Italic</span>'
      );
    });

    it("should handle options without wrapping div", () => {
      const content: TextContent[] = [{ text: "No Wrapper", style: {} }];
      ...
      expect(html).toBe(
        '<p style=""><span style="font-size:23.99px">No Wrapper</span></p>'
      );
    });

    it("should handle custom div style", () => {
      const content: TextContent[] = [{ text: "Custom Div", style: {} }];

      const options: HtmlConversionOptions = {
        divStyle: "margin: 10px; padding: 5px",
      };
      const html = HtmlConverter.convertSingleParagraphToHtml(content, options);
    ...
    it("should handle custom paragraph style", () => {
      const content: TextContent[] = [{ text: "Custom Paragraph", style: {} }];

      const options: HtmlConversionOptions = {
        paragraphStyle: "line-height: 1.5",
      };
      const html = HtmlConverter.convertSingleParagraphToHtml(content, options);
    ...
    it("should handle text alignment from options", () => {
      const content: TextContent[] = [{ text: "Right Aligned", style: {} }];
  ...
  describe("Multiple Paragraphs Conversion", () => {
    it("should convert multiple paragraphs to HTML", () => {
      ...
      expect(html).toContain(
        '<p style=""><span style="font-size:23.99px">First paragraph</span></p>'
      );
      expect(html).toContain(
        '<p style=""><span style="font-size:23.99px">Second paragraph</span></p>'
      );
      expect(html).toContain(
        '<p style=""><span style="font-size:23.99px">Third paragraph</span></p>'
      );
    });
    ...
    it("should handle options without wrapping div for multiple paragraphs", () => {
      ...
      expect(html).toBe(
        '<p style=""><span style="font-size:23.99px">Para 1</span></p><p style=""><span style="font-size:23.99px">Para 2</span></p>'
      );
    });
  });
  ...
  describe("Theme Color Handling", () => {
    ...
    it("should handle explicit theme color types", () => {
      const content: TextContent[] = [
        {
          text: "Theme Color",
          style: { color: "rgba(255,0,0,1)", themeColorType: "accent2" },
        },
      ];
  ...
  describe("HTML Escaping", () => {
    ...
    it("should handle all HTML special characters", () => {
      const content: TextContent[] = [{ text: "&<>\"'", style: {} }];
    ...
    it("should handle null and undefined text gracefully", () => {
      const content: TextContent[] = [{ text: null as any, style: {} }];
  ...
  describe("Style Ordering and Formatting", () => {
    ...
    it("should handle empty styles gracefully", () => {
      const content: TextContent[] = [{ text: "No Style", style: {} }];
  ...
  describe("Edge Cases and Error Handling", () => {
    it("should handle content with only empty text", () => {
      ...
      // 空文本内容也会有默认字体大小
      expect(html).toBe(
        '<div style=""><p style=""><span style="font-size:23.99px"></span></p></div>'
      );
    });
    ...
    it("should handle very long text content", () => {
      ...
      const content: TextContent[] = [{ text: longText, style: {} }];
    ...
    it("should handle special characters in text", () => {
      ...
      const content: TextContent[] = [{ text: specialText, style: {} }];
    ...
    it("should handle rich text with mixed formatting", () => {
      const paragraphs = [
        [
          {
            text: "Title: ",
            style: { bold: true, fontSize: 18, color: "#4472C4" },
          },
          {
            text: "PowerPoint to PPTist Converter",
        ...
        [
          {
            text: "Description: ",
            style: { underline: true, color: "#333333" },
          },
          { text: "This tool converts ", style: {} },
          {
            text: "PPTX files",
            style: { bold: true, backgroundColor: "rgba(255,255,0,0.3)" },
          },
          { text: " to ", style: {} },
          {
            text: "PPTist-compatible JSON",
            style: { italic: true, fontFamily: "Courier New" },
          },
          { text: " format.", style: {} },
        ],
      ];
    ...
    it("should handle presentation slide with title and bullet points", () => {
      const paragraphs = [
        [
          {
            text: "Main Title",
            style: { bold: true, fontSize: 24, color: "#4472C4" },
          },
        ],
        [{ text: "• First bullet point", style: { fontSize: 16 } }],
        [
          { text: "• Second bullet point with ", style: { fontSize: 16 } },
          { text: "emphasis", style: { italic: true, fontSize: 16 } },
        ],
