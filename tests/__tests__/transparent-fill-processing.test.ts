...
describe("透明填充处理测试", () => {
  ...
  describe("基本透明填充功能", () => {
    it("应该将小图片用透明背景填充到容器尺寸", async () => {
      ...
      // 检查是否有任何图像处理相关的效果
      const hasImageProcessingEffect = result.appliedEffects.some(effect => 
        effect.includes("transparent") || 
        effect.includes("padding") || 
        effect.includes("Applied fillRect transform") ||
        effect.includes("fillRect resize") ||
        effect.includes("fillRect stretch")
      );
      
      expect(hasImageProcessingEffect).toBe(true);
    });
    ...
    it("应该处理源图裁剪并用透明背景填充", async () => {
      ...
      const testImage = await createLargeTestImageBuffer();
      const config: StretchOffsetConfig = {
        containerWidth: 250,
        containerHeight: 180,
      ...
      expect(result.appliedEffects.some(effect => effect.startsWith("srcRect crop:"))).toBe(true);
    });
  });
  ...
  describe("复合效果测试", () => {
    it("应该同时处理srcRect裁剪和fillRect拉伸偏移", async () => {
      if (!processor.isAvailable()) {
        console.warn(
          "⚠️ Sharp not available, skipping composite effects tests"
        );
        return;
      }
  ...
  describe("错误处理", () => {
    ...
    it("应该处理无效的容器尺寸", async () => {
      if (!processor.isAvailable()) {
        console.warn(
          "⚠️ Sharp not available, skipping invalid container tests"
        );
        return;
      }
      ...
      await expect(
        processor.applyStretchOffset(testImage, config)
      ).rejects.toThrow("Expected valid width, height and channels to create a new input image");
    });
  });
});
