...
describe('PPTX 元素类型验证', () => {
  ...
  describe('文本元素', () => {
    it('应该正确解析文本内容', () => {
      ...
      if (expectedTextElements.length > 0) {
        expect(actualTextElements.length).toBeGreaterThanOrEqual(expectedTextElements.length * 0.8); // 允许一些容错
      }
    });
    ...
    it('应该正确解析图像源', () => {
      ...
      if (imageElements.length > 0) {
        imageElements.forEach((imageEl: any) => {
          ...
          // 应该是有效的URL或base64数据
          const isUrl = imageEl.src.startsWith('http') || imageEl.src.startsWith('https');
  ...
  describe('元素关系', () => {
    it('应该保留元素层级顺序', () => {
      ...
      actualOutput.slides.forEach((slide: any) => {
        const elementsWithIndex = slide.elements.filter((el: any) => el.index !== undefined);
    ...
    it('应该正确处理分组元素', () => {
      ...
      actualOutput.slides.forEach((slide: any) => {
        ...
        Object.values(yGroups).forEach(group => {
          if (group.length > 1) {
            const leftPositions = group.map(el => el.left).sort((a, b) => a - b);
            // 验证元素不会显著重叠
            for (let i = 1; i < leftPositions.length; i++) {
              const prevElement = group.find(el => el.left === leftPositions[i-1]);
              const currElement = group.find(el => el.left === leftPositions[i]);
