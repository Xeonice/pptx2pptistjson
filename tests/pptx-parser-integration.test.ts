...
describe('PPTX 解析器集成测试', () => {
  ...
  describe('幻灯片内容验证', () => {
    ...
    it('应该具有含必需属性的形状元素', () => {
      ...
      actualOutput.slides.forEach((slide: any) => {
        const shapeElements = slide.elements.filter((el: any) => el.type === 'shape');
  ...
  describe('期望输出比较', () => {
    it('应该匹配期望的结构键', () => {
      ...
      expect(hasTitle !== undefined || actualOutput.slides.length > 0).toBe(true);
    });
    ...
    it('应该具有相似的元素类型分布', () => {
      ...
      expectedOutput.slides.forEach((slide: any) => {
        slide.elements.forEach((element: any) => {
          expectedElementTypes[element.type] = (expectedElementTypes[element.type] || 0) + 1;
        });
      });
      ...
      actualOutput.slides.forEach((slide: any) => {
        slide.elements.forEach((element: any) => {
          actualElementTypes[element.type] = (actualElementTypes[element.type] || 0) + 1;
        });
      });
