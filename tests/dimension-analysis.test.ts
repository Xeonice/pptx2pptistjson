...
describe("尺寸转换逻辑分析", () => {
  ...
  describe("尺寸转换问题诊断", () => {
    it("应该分析所有元素的尺寸转换比例", () => {
      ...
      expectedOutput.slides.forEach(
        (expectedSlide: any, slideIndex: number) => {
          ...
          const minCount = Math.min(
            expectedSlide.elements.length,
            actualSlide.elements.length
          );
          ...
          for (let i = 0; i < minCount; i++) {
            ...
            console.log(
              `    期望尺寸: ${expectedElement.width.toFixed(
                1
              )} × ${expectedElement.height.toFixed(1)}`
            );
            console.log(
              `    实际尺寸: ${actualElement.width.toFixed(
                1
              )} × ${actualElement.height.toFixed(1)}`
            );
            console.log(
              `    宽度比例: ${widthRatio.toFixed(
                3
              )} (差异: ${widthDiff.toFixed(1)}px)`
            );
            console.log(
              `    高度比例: ${heightRatio.toFixed(
                3
              )} (差异: ${heightDiff.toFixed(1)}px)`
            );
            console.log("");
          }
        }
      ...
      const avgWidthRatio =
        widthRatios.reduce((sum, ratio) => sum + ratio, 0) / widthRatios.length;
      const avgHeightRatio =
        heightRatios.reduce((sum, ratio) => sum + ratio, 0) /
        heightRatios.length;
      ...
      console.log(
        `宽度比例范围: ${minWidthRatio.toFixed(3)} - ${maxWidthRatio.toFixed(
          3
        )}`
      );
      console.log(
        `高度比例范围: ${minHeightRatio.toFixed(3)} - ${maxHeightRatio.toFixed(
          3
        )}`
      );
      ...
      Object.entries(typeGroups).forEach(([type, items]) => {
        ...
        const avgTypeWidthRatio =
          typeWidthRatios.reduce((sum, ratio) => sum + ratio, 0) /
          typeWidthRatios.length;
        const avgTypeHeightRatio =
          typeHeightRatios.reduce((sum, ratio) => sum + ratio, 0) /
          typeHeightRatios.length;
      ...
      const widthVariance =
        widthRatios.reduce(
          (sum, ratio) => sum + Math.pow(ratio - avgWidthRatio, 2),
          0
        ) / widthRatios.length;
      const heightVariance =
        heightRatios.reduce(
          (sum, ratio) => sum + Math.pow(ratio - avgHeightRatio, 2),
          0
        ) / heightRatios.length;
      ...
      console.log(
        `如果实际尺寸是 EMU，转换到点后的期望比例: ${EMU_TO_POINTS.toFixed(6)}`
      );
    ...
    it("应该提供转换函数修复建议", () => {
      ...
      const { avgWidthRatio, avgHeightRatio, isConsistentRatio, typeGroups } =
        analysisResult;

      if (isConsistentRatio) {
        console.log("✅ 检测到一致的转换比例，建议应用全局转换因子:");
        console.log(
          `   宽度: actualWidth *= ${(1 / avgWidthRatio).toFixed(3)}`
        );
        console.log(
          `   高度: actualHeight *= ${(1 / avgHeightRatio).toFixed(3)}`
        );
        console.log("");
        console.log("实施方法:");
        ...
        Object.entries(typeGroups).forEach(([type, items]) => {
          const typeWidthRatios = (items as any[]).map(
            (item) => item.widthRatio
          );
          const typeHeightRatios = (items as any[]).map(
            (item) => item.heightRatio
          );
